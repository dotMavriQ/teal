    ...
    <header class="bg-theme-bg-primary shadow">
        <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-2">
                ...
                <p class="mt-1 text-sm text-theme-text-muted">Fetch and apply missing book metadata from OpenLibrary.</p>
            </div>
        </div>
    </header>
                ...
                <div class="px-4 py-5 sm:p-6">
                    ...
                    <div class="mt-4 space-y-2">
                        @foreach($sourcePriority as $index => $source)
                            <div class="flex items-center justify-between p-3 bg-theme-bg-tertiary rounded-lg border border-theme-border-secondary">
                                <div class="flex items-center gap-3">
                                    <span class="flex items-center justify-center w-6 h-6 rounded-full bg-theme-accent-primary/10 text-theme-accent-primary text-sm font-medium">
                                        {{ $index + 1 }}
                                    </span>
                                    <span class="font-medium text-theme-text-primary">{{ $this->getSourceLabel($source) }}</span>
                                    @if($index === 0)
                                        <span class="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-theme-success">
                                            Highest Priority
                                        </span>
                                    @endif
                                </div>
                    ...
                    <div class="px-4 py-5 sm:p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-theme-text-muted">Metadata enrichment in progress...</p>
                            </div>
                            @if($jobStatus['status'] === 'running')
                                <div class="flex items-center gap-2">
                                    <svg class="animate-spin h-6 w-6 text-theme-accent-primary" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <span class="text-sm font-medium text-theme-accent-primary">Running</span>
                                </div>
                        ...
                        <div class="mt-4">
                            ...
                            <div class="w-full bg-theme-bg-tertiary rounded-full h-2">
                                <div
                                    class="bg-theme-accent-primary h-2 rounded-full transition-all duration-500"
                        ...
                        <div class="mt-4 grid grid-cols-3 gap-4">
                            <div class="text-center">
                                <div class="text-2xl font-bold text-theme-accent-primary">{{ $jobStatus['fetched'] ?? 0 }}</div>
                    ...
                    @if($hasScanned)
                        <div class="mt-4 flex flex-wrap gap-4 text-sm">
                            ...
                            @if(!empty($fetchedData))
                                <div class="flex items-center gap-2">
                                    <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-green-100 text-theme-success font-medium">
                                        {{ $this->getFetchedCount() }}
                                    </span>
                                    <span class="text-theme-text-secondary">Fetched from OpenLibrary</span>
                                </div>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-theme-border-secondary">
                            <thead class="bg-theme-bg-tertiary">
                                <tr>
                                    <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-theme-text-primary sm:pl-6">Book</th>
                            ...
                            <tbody class="divide-y divide-theme-border-secondary bg-theme-card-bg">
                                @foreach($booksNeedingEnrichment as $book)
                                    @if($book['has_missing'])
                                    <tr wire:key="book-row-{{ $book['id'] }}">
                                        <td class="py-4 pl-4 pr-3 text-sm sm:pl-6">
                                            <div class="font-medium text-theme-text-primary">{{ $book['title'] }}</div>
                                            <div class="text-gray-500">{{ $book['author'] }}</div>
                                            <div class="text-theme-text-muted text-xs font-mono">{{ $book['isbn'] }}</div>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm">
                                            @if(isset($fetchedData[$book['id']]))
                                                <span class="inline-flex items-center rounded-full bg-green-100 px-2 py-1 text-xs font-medium text-theme-success">
                                        ...
                                        <td class="px-3 py-4 text-sm text-theme-text-muted">
                                        ...
                                        <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                            <button
                                                ...
                                                type="button"
                                                class="text-theme-accent-primary hover:text-theme-accent-primary/70"
                                            >
                                                Review
                                            </button>
                                        </td>
            ...
            @elseif($hasScanned)
                <div class="bg-theme-card-bg shadow rounded-lg">
                    <div class="px-4 py-12 text-center">
                        ...
                        <h3 class="mt-2 text-sm font-semibold text-theme-text-primary">No books with ISBN found</h3>
                        <p class="mt-1 text-sm text-theme-text-muted">Import books with ISBNs to use metadata enrichment.</p>
                    </div>
                </div>
            @endif
        </div>
            ...
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-theme-card-bg px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-theme-text-muted">{{ $reviewingBook['author'] }}</p>
                                ...
                                <table class="min-w-full divide-y divide-theme-border-secondary">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-theme-text-primary">OpenLibrary</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-theme-border-secondary">
                                        ...
                                        @foreach($fields as $field => $label)
                                            ...
                                            <tr class="{{ $hasConflict ? 'bg-amber-50' : '' }}">
                                                ...
                                                <td class="px-3 py-4 text-sm text-theme-text-muted max-w-[200px]">
