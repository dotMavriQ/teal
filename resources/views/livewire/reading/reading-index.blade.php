                ...
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-4 text-sm font-medium text-theme-text-tertiary" aria-current="page">Reading</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2" role="list">
                @foreach($subcategories as $category)
                    ...
                    @else
                        <div
                            class="relative flex flex-col items-center rounded-lg border-2 border-theme-border-primary bg-theme-bg-tertiary p-8 opacity-60 cursor-not-allowed"
                            role="listitem"
                            aria-disabled="true"
                        >
                            <div class="flex h-20 w-20 items-center justify-center rounded-full bg-theme-bg-hover mb-4">
                                @if($category['icon'] === 'squares-2x2')
                                    <svg class="h-10 w-10 text-theme-text-muted" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" />
                                    </svg>
                                @endif
                            </div>
                            <h3 class="text-2xl font-semibold text-theme-text-tertiary">{{ $category['name'] }}</h3>
                            <p class="mt-1 text-sm text-theme-text-muted text-center">{{ $category['description'] }}</p>
                            <span class="mt-3 inline-flex items-center rounded-full bg-theme-bg-hover px-3 py-1 text-sm font-medium text-theme-text-secondary">
                                Coming Soon
                            </span>
                        </div>
                    @endif
