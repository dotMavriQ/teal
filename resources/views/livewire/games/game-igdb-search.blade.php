                                    ...
                                    @if(!empty($result['platforms']))
                                        <div class="mt-1 flex flex-wrap gap-0.5">
                                            @foreach(array_slice($result['platforms'], 0, 2) as $plat)
                                                <span class="inline-block rounded-sm px-1 py-0.5 text-[8px] font-medium bg-theme-bg-tertiary text-theme-text-muted border border-theme-border-primary">{{ $plat }}</span>
                                            @endforeach
                                            @if(count($result['platforms']) > 2)
                                                <span class="inline-block rounded-sm px-1 py-0.5 text-[8px] text-theme-text-muted">+{{ count($result['platforms']) - 2 }}</span>
                                            @endif
                                        </div>
                                    @endif
                                </div>
                        ...
                        <div class="p-6">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    >
                                    <button type="button" wire:click="addCustomPlatform" class="rounded-md btn-secondary px-3 py-1.5 text-sm font-medium ring-1 ring-inset shadow-xs">Add</button>
                                </div>
                                        ...
                                        @foreach($selectedPlatforms as $index => $platform)
                                            @if(!in_array($platform, $availablePlatforms))
                                                <span class="inline-flex items-center gap-1 rounded-full bg-theme-accent-primary text-theme-text-inverted px-3 py-1 text-sm font-medium">
                                                    {{ $platform }}
                                                    <button type="button" wire:click="removeSelectedPlatform({{ $index }})" class="ml-1 hover:opacity-75 focus:outline-hidden" aria-label="Remove {{ $platform }}">
                            ...
                            <div class="flex items-center justify-end gap-3 pt-4 border-t border-theme-border-primary">
                                ...
                                <button wire:click="addGame" type="button" class="rounded-md btn-primary px-4 py-2 text-sm font-medium shadow-xs">
                                    Add to Library
                                </button>
                            </div>
                        </div>
