        ...
        <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('board-games.discover') }}" class="inline-flex items-center gap-1.5 rounded-md btn-secondary px-3 py-2 text-sm font-medium shadow-xs ring-1 ring-inset">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg>
                        <span class="hidden sm:inline">Discover</span>
                    </a>
                    <a href="{{ route('board-games.create') }}" class="inline-flex items-center gap-1.5 rounded-md btn-primary px-3 py-2 text-sm font-medium shadow-xs">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @if($boardGames->isEmpty())
                <div class="text-center py-16">
                    ...
                    <div class="mt-6 flex justify-center gap-3">
                        <a href="{{ route('board-games.create') }}" class="rounded-md btn-primary px-4 py-2 text-sm font-medium shadow-xs">
                            Add Board Game
                        </a>
                        <a href="{{ route('board-games.discover') }}" class="rounded-md btn-secondary px-4 py-2 text-sm font-medium shadow-xs ring-1 ring-inset">
                            Discover
                        </a>
                    </div>
                </div>
                    ...
                    @if($viewMode === 'gallery')
                        ...
                        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                            @foreach($boardGames as $boardGame)
                                <article wire:key="board-game-{{ $boardGame->id }}" class="group relative bg-theme-card-bg rounded-lg shadow-xs ring-1 ring-theme-border-primary overflow-hidden hover:shadow-md transition-shadow">
                                    <div class="absolute top-2 left-2 z-10">
                                        <input wire:model.live="selected" type="checkbox" value="{{ $boardGame->id }}" class="h-4 w-4 rounded-sm border-theme-border-secondary text-theme-accent-primary bg-white/90 shadow-xs">
                                    </div>
                                    @if($boardGame->rating)
                                        ...
                                        <tr>
                                            ...
                                            <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-theme-text-tertiary uppercase tracking-wider hidden lg:table-cell">
                                                <button wire:click="sort('plays')" class="group inline-flex items-center gap-1 hover:text-theme-text-primary">
                                                    Plays
                                                    <span class="flex-none rounded-sm {{ $sortBy === 'plays' ? 'text-theme-text-primary' : 'text-theme-text-muted invisible group-hover:visible' }}">
                                    ...
                                    <tbody class="bg-theme-card-bg divide-y divide-theme-border-primary">
                                        @foreach($boardGames as $boardGame)
                                            <tr wire:key="board-game-{{ $boardGame->id }}" class="hover:bg-theme-bg-hover">
                                                <td class="px-3 py-2">
                                                    <input wire:model.live="selected" type="checkbox" value="{{ $boardGame->id }}" class="h-4 w-4 rounded-sm border-theme-border-secondary text-theme-accent-primary">
                                                </td>
                                                <td class="px-2 py-2">
                                                    <a href="{{ route('board-games.show', $boardGame) }}" class="block">
